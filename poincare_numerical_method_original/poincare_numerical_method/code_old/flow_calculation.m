function [cumQ]=flow_calculation(k,k10,xi,cycle,lostTime, vf,kj, kc, L1)
...
nLostTime=lostTime/ddt; %total steps for the lost time in each phase
N=NT;
M=1;
...
for i=1:N
    ...
    if(mod(i,NT)<=NT/2-nLostTime)
        signal=1;
    elseif(mod(i,NT)>NT/2 &&mod(i,NT)<=NT-nLostTime)
        signal=2;
    else
        signal=0;
    end
