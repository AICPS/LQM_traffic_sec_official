function [cumQa, cumQ, k1_ret, k2_ret]=LQM_grid_attack(k,k10,xi1,xi2,cycle,Tsim,Tstart,Tend,gT1,gT2,gT1_d,gT2_d,vf,kj,kc,L1,W,H)
...
k20=2*k-k10;
% for each time step, need to update the flows and the densities in the
% links
...
% Attack settings
Nstart = Tstart*cycle*(1/ddt);
Nend = Tend*cycle*(1/ddt);
...
for t = 1:N
    ...
    if(t >= Nstart && t <= Nend) % attack window
        nGreen1=(gT1_d*cycle)/ddt;
        nLostTime=(1-gT1_d-gT2_d)*cycle/2/ddt;
    else 
        nGreen1=(gT1*cycle)/ddt; % no attack
        nLostTime=(1-gT1-gT2)*cycle/2/ddt;
    end
...
%         fprintf('Attack NT is: %.3f\n', NT);
%         fprintf('Attack: current iteration is: %.10f\n', t);
        cumQ = ((sum(sum(g1(t-(NT):t,:))/(H*W))/(NT)+ sum(sum(g2(t-(NT):t,:))/(H*W))/(NT)))/2;
%         fprintf('Attack: Flow is: %.10f\n', cumQ);
        cumQa = [cumQa cumQ];
    end

    %cumQ = ((sum(sum(g1*dt)/(H*W))/N + sum(sum(g2*dt)/(H*W))/N))*3600;
%     if(t > NT)
...
% disp(size(g1(N-NT:N,:)));
%disp(g1(:, :));
%cumQ = (sum(sum(g1(N-NT:N,:))))*dt/(cycle*H*W)*3600;
