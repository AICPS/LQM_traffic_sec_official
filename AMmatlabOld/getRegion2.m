function [R2, k12]=getRegion2(k,k11,T,g2,xi2,kc,kj,vf,L)
...
if(k>(kj+kc)/2) 
    %% regions 7, 8
    bound2=2*k*(1-xi2)-(1-2*xi2)*kj; %maximum bound between region 7 and 8
    
    %% for phase 2    
    if(k11<bound2) %in region 7?
        k1_tmp=(kj-2*k+k11)*exp(gamma5*greenT2)-(kj-2*k);
        if(k1_tmp<=bound2) %do not cross region 7
            k12=k1_tmp; %k1 temp is the updated value of k1 after green T2 according to Poincare map
            R2=7;
        else %go to region 8
            t2=log((kj-2*k+bound2)/(kj-2*k+k11))/gamma5; %time it takes to become max
            dt=greenT2-t2;
            k12=kj+(bound2-kj)*exp(-gamma3*dt); %update k1 according to 
            R2=8;
        end 
    else %in region 8
