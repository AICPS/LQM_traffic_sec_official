...
def main(pi1, pi2, pi1_d, pi2_d, xi1, xi2):
	pi1_d = 0.467
	pi2_d = 0.467
	...
	while return_deo(k_1,k) != (3,8):
		for k in np.arange(k_c, (k_j+k_c)/2, 2):
			for k_1 in np.arange(k_c, k_j - xi1*(k_j-k_c), 2):
				...
				print('k1 max for region 2: ', k_j - (xi1)*(k_j-k_c))
				print('k is set to: ', k)
	...
	# def_regions(k_j - (1-xi2)*(k_j-k_c), k)
