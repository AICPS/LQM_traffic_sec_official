...
function [vuln_k1, vuln_k, vuln_xi, deo_01, deo_02, deo_final1d, deo_final2d]=...
    GTR_attack_model(xi1, xi2, gT1, gT2, gT1_d, gT2_d, ...
...
%     target_states = cat(1, [3,8], [4,7]);
    ...
    %% Possible intermediary states include: (3,7) or (3,6) for xi > 0.5 
    ...
    %% ALGORITHM START
    % Writing Settings
%     fname = strcat('gtrattack_drn_4-8_', num2str(gT1), '_', num2str(gT2), ...
%         '_', num2str(gT1_d), '_', num2str(gT2_d), '_', num2str(T_start), ...
%         '_', num2str(T_end), '_', num2str(N), '_', num2str(T), '_', num2str(xi1));
                        ...
                        for n = 1:N 
                            ...
                            if(k_2 <= kj && k_2d <= kj) % make sure we don't test unreasonable states
                                if (n > T_start && n <= T_end) % attack window
                                    [q, k1_next, k2] = ...
                                        flow_calculation(k,k1_prev,xi1,xi2,T,gT1,gT2, vf, kj, kc, L1);
                                    [q_d, k1_d_next, k2] = ...
                                        flow_calculation(k,k1_d_prev,xi1,xi2,T,gT1_d,gT2_d, vf, kj, kc, L1);
                                    q_array = [q_array q];
                                    ...
                                    [q, k1_next, k2] = ...
                                        flow_calculation(k,k1_prev,xi1,xi2,T,gT1,gT2, vf, kj, kc, L1);
                                    [q_d, k1_d_next, k2] = ...
                                        flow_calculation(k,k1_d_prev,xi1,xi2,T,gT1,gT2, vf, kj, kc, L1);
                                    q_array = [q_array q];
                                    q_array_d = [q_array_d q_d];
                                    k1_array = [k1_array k1_next];
                                    k1_d_array = [k1_d_array k1_d_next];
                                    ...
                                    if(mod(n, deo_period) == 0)
                                        [r1,r2] = ...
                                            getDEO(xi1, xi2, gT1, gT2, k1_prev, k,T);
                                        [r1d,r2d] = ...
                                            getDEO(xi1, xi2, gT1, gT2, k1_d_prev, k,T);
                                        deos = [deos  [r1,r2]];
                                        deos_d = [deos_d [r1d,r2d]];
                                    end
                                end
